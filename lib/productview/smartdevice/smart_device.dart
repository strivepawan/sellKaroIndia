...
class PhoneAdCards extends StatefulWidget {
  const PhoneAdCards({super.key});

  @override
  State<PhoneAdCards> createState() => _ElectronicAndAppliencesState();
}

class _ElectronicAndAppliencesState extends State<PhoneAdCards> {
  late List<AdData> adsData = [];
            ...
            StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  adsData = snapshot.data!.docs.map((DocumentSnapshot document) {
                        ...
                        return GestureDetector(
                          onTap: () {
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ElectronicProductView(
                                  adId: adsData[index].id,
                                  catDocid: 'Device',
                                ),
                              ),
                            );
  ...
  Stream<QuerySnapshot> fetchAdsDataStream() {
    try {
      ...
      Stream<QuerySnapshot> categoryStream =
          adsCollectionReference.orderBy('timestamp', descending: true).snapshots();
...
class AdCard extends StatefulWidget {
  ...
  const AdCard({Key? key, required this.adData}) : super(key: key);
        ...
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Text(
                'RS ${widget.adData.price.toString()}',
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
                  color: Colors.green,
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      widget.adData.userAddress,
                      style: const TextStyle(
                        color: Color.fromARGB(118, 126, 148, 1),
                        fontSize: 12,
                      ),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
